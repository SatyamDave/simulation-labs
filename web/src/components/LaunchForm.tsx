...
// Memory mode governs whether personas recall what past runs learned. "off" is
// the honest research default (personas behave as genuine first-time users).
const MEMORY_MODES: { id: MemoryMode; label: string; help: string }[] = [
  {
    id: "off",
    label: "Fresh users",
    help: "No memory — personas behave as first-time visitors (honest research default).",
  },
  {
    id: "site_hints",
    label: "Site hints",
    help: "Reuse what past runs learned about this site — fewer steps, lower cost.",
  },
  {
    id: "returning_user",
    label: "Returning",
    help: "Each persona also recalls its own prior visits to this site.",
  },
];
    ...
    <section className="px-6 pt-20 pb-24 md:pt-28">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            <span className={FIELD_LABEL}>Memory</span>
            <div className="grid grid-cols-3 gap-2">
              {MEMORY_MODES.map((m) => {
                const on = memoryMode === m.id;
                return (
                  <button
                    type="button"
                    key={m.id}
            ...
            <span className="text-xs text-muted-foreground/60 px-1">
              {MEMORY_MODES.find((m) => m.id === memoryMode)?.help}
            </span>
          </div>
